...
fn extract_frame_number(frame_layer: &Value) -> Option<u64> {
    if let Value::Object(map) = frame_layer {
        let candidates = ["frame.number", "frame.num", "frame.frame_number", "frame.no"];
        for k in candidates {
...
fn packet_to_record(v: &Value, include_raw_hex: bool) -> Option<CamRecord> {
    ...
    let frame_number = layers.get("frame").and_then(extract_frame_number);
...
fn post_cloudevent_structured(
    ...
    if !resp.status().is_success() {
        return Err(
            format!("sink returned non-success HTTP status {}", resp.status()).into(),
        );
    }
...
fn sender_worker(
    ...
    for rec in rx {
        if let Err(e) = post_cloudevent_structured(&client, &sink_url, &event_type, &source, &rec) {
...
fn run_live() -> Result<(), Box<dyn Error>> {
    ...
    let stdout = child.stdout.take().expect("failed to capture tshark stdout");
    ...
    for line_res in reader.lines() {
        ...
        let json_val: Value = match serde_json::from_str(trimmed) {
            ...
            Err(e) => {
                warn!(
                    "failed to parse JSON from tshark: {} | line={}",
                    e, trimmed
                );
                continue;
            }
